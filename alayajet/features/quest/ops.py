...
def decode_sparse_attn(
    ...
    # 1. Vectorized Disk Load
    # physical_indices: (H_q, k)
    # kv_head_indices: (H_q, 1)
    kv_head_indices = (np.arange(num_heads, dtype=np.int64) // group_size)[:, None]
    
    # disk_pool shape: (Layers, Capacity, 2, PageSize, H_kv, D)
    # We index: layer_idx, physical_indices, :, :, kv_head_indices, :
    # Result shape: (H_q, k, 2, PageSize, D)
    
    pages_np = controller.kv_cache.disk_pool[layer_idx, physical_indices, :, :, kv_head_indices, :]
